...
class GeminiService:
  def __init__(self, api_key=None):
    ...
    self.api_url= (f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={self.api_key}" if self.api_key else None)
    ...
    def _call_gemini(self, prompt):
      ...
      payload={"contents":[{"parts": [{"text": prompt}]}], "generationConfig": {"temperature": 0.2, "maxOutputTokens": 1000} }
    ...
    def get_or_generate_questions(self, skill_name: str, count: int = 5):
        ...
        if raw_ai:
            try:
                ...
                questions = []
        # 3. Smart Offline Fallback (if no API key or network error)
        if not questions:
            questions = [
                {
        ...
        # 4. Save into SQLite Cache for future reuse!
        saved_questions = []
    ...
    def grade_assessment(self, student_id: int, skill_name: str, submitted_answers: dict):
        ...
        conn = get_db()
        cursor = conn.cursor()
        # Grade each question against the answer key
        correct_count = 0
        ...
        # Save to student_skill_scores (Update if already exists!)
        cursor.execute(
            """
    ...
    def get_or_generate_fit_score(self, student_id: int, posting_id: int):
        ...
        cursor = conn.cursor()
        # 1. Check Cache
        cursor.execute("SELECT fit_score, gap_analysis_text FROM skill_gap_cache WHERE student_id = ? AND posting_id = ?", (student_id, posting_id))
        ...
        # 3. Call Gemini
        prompt = (
            f"Evaluate candidate student {student['name']} (Skills: {student['skills']}, Experience: {student['prior_experience']}) "
            f"against opportunity '{posting['title']}' (Required: {posting['required_skills']}, Description: {posting['description']}). "
        ...
        fit_score = 85.0
        analysis = f"Good match on core skills ({student.get('skills')}). Recommendation: Strengthen practical project experience in {posting.get('required_skills')}."
        raw_ai = self._call_gemini(prompt)
        if raw_ai:
            try:
                clean = raw_ai.replace("```json", "").replace("```", "").strip()
    ...
    def get_or_generate_courses(self, student_id: int, posting_id: int):
        ...
        cursor = conn.cursor()
        # 1. Check Cache
        cursor.execute("SELECT recommended_courses FROM course_recommendations WHERE student_id = ? AND posting_id = ?", (student_id, posting_id))
        cached = cursor.fetchone()
        if cached:
            conn.close()
            return {"courses": json.loads(cached['recommended_courses']), "cached": True}
        # 2. Gather student & posting info
